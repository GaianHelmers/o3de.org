...
**Notable features:** The only template with no registration commands -- since it produces a data file rather than a C++ class, there is nothing to add to CMake or a module descriptor. All four commands are the same `replace_text` pattern applied to different placeholder tokens inside the `.attimage` file.
